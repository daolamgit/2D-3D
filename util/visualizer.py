...
class Visualizer():
    ''' Visualizer class borrowed from pix2pix.'''
    ...
    def __init__(self, opt):
        ...
        if '3d' in opt.which_model_netG:
            ...
            self.ffmpeg_params = {
                '-r': '10',
                #'-pix_fmt': 'yuv420p'  # haven't tested
            }
        else:
        ...
        if self.use_html and (save_result
            ...
            if self.video_mode:
                for label, video_numpy in visuals.items():
                    vid_path = os.path.join(self.img_dir, 'epoch%.3d_%s.mp4' %
                                            (epoch, label))
                    # video_numpy = util.rescale_vid(video_numpy, 2)
                    vwrite(vid_path, video_numpy, inputdict=self.ffmpeg_params)
            else:
                ...
                for label, image_numpy in visuals.items():
                    if self.video_mode:
                        vid_path = os.path.join(
                            self.img_dir, 'epoch%.3d_%s.mp4' % (epoch, label))
                        self.vis.video(
                            videofile=vid_path,
    ...
    def plot_current_errors(self, epoch, counter_ratio, opt, errors):
        ...
        self.plot_data['Y'].append(
            [errors[k] for k in self.plot_data['legend']])
        self.vis.line(
            X=np.stack([np.array(self.plot_data['X'])] * len(
                self.plot_data['legend']), 1),
    ...
    def print_current_errors(self, epoch, i, errors, t, t_data):
        message = '(epoch: %d, iters: %d, time: %.3f, data: %.3f) ' % (epoch,
                                                                       i, t,
                                                                       t_data)
