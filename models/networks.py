...
def define_G(input_nc,
    ...
    if which_model_netG == 'resnet_9blocks':
        ...
        #
        # netG = FCDenseNet(
        # in_channels=input_nc,    down_blocks = (4, 5, 7),
        # up_blocks = (7, 5, 4), bottleneck_layers = 15,
        # growth_rate = 16, out_chans_first_conv = 48, n_classes =1)


    elif which_model_netG == 'maskunet_128':
        netG = MaskUnetGenerator(
            input_nc,
            output_nc,
            7,
            ngf,
...
#put a conv infront of Unet
class HeadUnetGenerator(nn.Module):
    ''' Defines the UNet generator architecture proposed in Isola et al.
    Architecture contains downsampling/upsampling operations, with Unet
    connectors.
...
class MaskUnetGenerator(nn.Module):
    ...
    def __init__(self,
        ...
        self.convMask = nn.Conv2d( input_nc + 1, 1, kernel_size=3, padding=1)
        self.Sig = nn.Sigmoid()
...
class NLayerDiscriminator(nn.Module):
    ...
    def forward(self, input):
        if len(self.gpu_ids)>1 and isinstance(input.data,
